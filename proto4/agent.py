...
from base_dir.proto4.goal_policy import GoalSpaceSWM, select_goal, update_goal_space_swm
from base_dir.proto4.knowledge_base import KnowledgeBase, RolloutRecord
from base_dir.proto4.qlearning_policy import QLearningPolicy
...
class IMGEPAgent(Agent):
    def __init__(self, env: OvercookedEnv, mdp: OvercookedGridworld, agent_id: int, config: AgentConfig):
    ...
    def reset(self, mdp: Optional[OvercookedGridworld] = None,
              exploit: bool = False,
        ...
        # Ensure main_goal is always a GoalSpace object
        if chosen is None:
            self.main_goal = random.choice(self.goal_spaces)
        elif isinstance(chosen, tuple):
            self.main_goal = chosen[0]  # Take first element only
        else:
            self.main_goal = chosen
    ...
    def action(self, state: OvercookedState) -> Action:
        ...
        if self.previous_state and goal and hasattr(goal, "success") and goal.success(
            self.agent_id, state, self.previous_state, self.mdp
        ) and self.goal_reach_time_step is None:
            self.goal_reach_time_step = self.t

        # Fitness accumulation
        if self.previous_state and goal and hasattr(goal, "fitness"):
        ...
        if not self.path:
            return self._return_action(random.choice(list(Action.MOTION_ACTIONS)), state)
