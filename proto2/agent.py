...
from base_dir.proto2.goal_policy import GoalSpaceSWM, select_goal, update_goal_space_swm
from base_dir.proto2.knowledge_base import KnowledgeBase, RolloutRecord
from base_dir.proto2.qlearning_policy import QLearningPolicy
...
class GoalPolicyWrapper:
    """
    Wrapper for a goal-specific Q-learning policy.
    Each instance represents a policy targeting a specific goal space.
    """
    def __init__(self, goal: GoalSpace, q_policy: QLearningPolicy, exploit: bool):
        self.goal = goal
...
class IMGEPAgent(Agent):
    def __init__(
        ...
        # ---------------- Goal-specific Q-learning policies ----------------
        self.goal_policy_stack: List[GoalPolicyWrapper] = []
        self.q_policy_dict = {}  # persistent Q-learning policies per goal
    ...
    def reset(self, mdp: Optional[OvercookedGridworld] = None, exploit: bool = False):
        ...
        # Select main goal
        chosen_goal, exploit = select_goal(
            self.goal_space_swms,
            goal_spaces=self.goal_spaces,
            exploit=exploit,
        )
        if chosen_goal is None:
            chosen_goal = random.choice(self.goal_spaces)

        # Initialize Q-learning policy for this goal if not already
        if chosen_goal.goal_id not in self.q_policy_dict:
            self.q_policy_dict[chosen_goal.goal_id] = QLearningPolicy(
                obs_dim=None,
        ...
        # Initialize goal_policy_stack
        self.goal_policy_stack = [
            GoalPolicyWrapper(
                goal=chosen_goal,
                q_policy=self.q_policy_dict[chosen_goal.goal_id],
                exploit=exploit,
            )
        ]
    ...
    def action(self, state: OvercookedState) -> Action:
        ...
        self.t += 1
        current_policy = self.goal_policy_stack[-1]
        goal = current_policy.goal
        exploit = current_policy.exploit
        ...
        if (
            ...
            if len(self.goal_policy_stack) == 1:
                self.goal_reach_time_step = self.t
            else:
                self.goal_policy_stack.pop()

        # Fitness accumulation (only for active goal)
        ...
        action_token = current_policy.q_policy.select_action(obs_vec, greedy=greedy)

        # Store transition from previous step
        if self.previous_obs is not None:
            current_policy.q_policy.store_transition(
                self.previous_obs,
            ...
            if self.t % 5 == 0 and len(current_policy.q_policy.replay_buffer) >= 32:
                for _ in range(2):  # Quick training steps during rollout
                    current_policy.q_policy.train_step()
    ...
    # -----------------------------------------------------
    def finish_rollout(self, info, partner_goal_id=None):
        current_policy = self.goal_policy_stack[0]
        goal = current_policy.goal
        q_policy = current_policy.q_policy
        exploit = current_policy.exploit
        ...
        self.kb.add_record(
            RolloutRecord(
                ...
                partner_goal_id=partner_goal_id if partner_goal_id is not None else -1,
