...
for roll in range(ROLLOUTS):
    ...
    for ag in agents:
        ag.reset(mdp=mdp, exploit=exploit)
    done = False
    state = env.state
    ...
    for idx, ag in enumerate(agents):
        # Use the main goal from the goal_policy_stack
        main_goal_id = ag.goal_policy_stack[0].goal.goal_id
        ag.finish_rollout(info, partner_goal_id=agents[1 - idx].goal_policy_stack[0].goal.goal_id)
...
base_dir = f"base_dir/proto2/layout{LAYOUT_ID}/buffer_rollouts"
